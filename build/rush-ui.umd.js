...
`)(),
    n = t.defineComponent({
      ...
      props: a,
      setup(o, { slots: r }) {
        const { size: d, type: b, disabled: l, block: u } = t.toRefs(o),
          f = t.computed(() => (u.value ? 's-btn--block' : ''))
        return () =>
          t.createVNode(
            'button',
            {
              class: `s-btn s-btn--${b.value} s-btn--${d.value} ${f.value}`,
              disabled: l.value
            },
            [r.default ? r.default() : '\u6309\u94AE']
          )
      }
    }),
    s = {
